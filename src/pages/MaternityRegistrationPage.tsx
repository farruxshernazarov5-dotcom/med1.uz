...
const MaternityRegistrationPage = () => {
  const { user } = useAuth();
      ...
      <section className="py-10 md:py-16">
        <div className="container mx-auto px-4 max-w-3xl">
          <div className="text-center mb-8">
            <div className="w-16 h-16 rounded-2xl bg-gradient-to-br from-pink-500 to-rose-400 flex items-center justify-center mx-auto mb-4">
              <Baby className="w-8 h-8 text-white" />
            </div>
            <h1 className="font-heading text-3xl font-bold text-foreground">Tug'ruqxona ro'yxatdan o'tish</h1>
            ...
            <CardContent className="space-y-4">
                ...
                <>
                  ...
                  <div>
                    ...
                    <div className="flex items-center gap-4 mt-1">
                      {logoPreview ? <img loading="lazy" decoding="async" src={logoPreview} className="w-16 h-16 rounded-xl object-cover border" alt="Klinika logotipi" /> : <div className="w-16 h-16 rounded-xl bg-muted flex items-center justify-center"><Camera className="w-6 h-6 text-muted-foreground" /></div>}
              ...
              {step === 1 && (
                <>
                  ...
                  <div>
                    ...
                    {innResult?.found && <p className="text-xs text-emerald-600 mt-1 flex items-center gap-1"><Check className="w-3 h-3" /> Topildi</p>}
                  </div>
              ...
              {step === 2 && (
                <>
                  ...
                  <div><Label className="flex items-center gap-2"><Globe className="w-4 h-4" /> Website</Label>
                    <Input value={form.website} onChange={(e) => update("website", e.target.value)} className="mt-1" /></div>
              ...
              {step === 4 && (
                <>
                  <p className="text-sm text-muted-foreground">Tug'ruqxonangizda mavjud xizmatlarni tanlang:</p>
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
                    {MATERNITY_SERVICES.map((s) => (
                    ...
                    {WEEKDAYS.map((day) => {
                      ...
                      return (
                        <div key={day.key} className="flex items-center gap-3 p-2 rounded-lg border border-border">
                          ...
                          {!wh.active && <span className="text-xs text-muted-foreground">Dam olish kuni</span>}
                        </div>
                      );
                    })}
                  </div>
              ...
              {step === 6 && (
                <div className="bg-accent/30 border border-accent rounded-xl p-5">
                  ...
                  <div className="space-y-2 text-sm">
                    {logoPreview && <img loading="lazy" decoding="async" src={logoPreview} className="w-16 h-16 rounded-xl object-cover border mb-2" alt="Klinika logotipi" />}
...
export default MaternityRegistrationPage;
