      ...
      if (totalPrice > 0) {
        supabase.from("invoices").insert({
          ...
          metadata: {
            ...
            "Klinika": selectedClinic?.name || "—",
            "Klinika manzili": selectedClinic?.address || "—",
            "Klinika telefon": selectedClinic?.phone || "—",
            "Shifokor": selectedDoctor?.full_name ? `Dr. ${selectedDoctor.full_name}` : "—",
            "Mutaxassislik": selectedDoctor?.specialty || "—",
            "Qabul sanasi": form.date,
            "Qabul vaqti": form.time,
            "Xizmat davomiyligi": selectedService?.duration_minutes ? `${selectedService.duration_minutes} daqiqa` : "—",
            "Izoh": form.notes || "—",
          },
        }).then(() => {});
      }
