...
import {
  HeartPulse, Loader2, AlertTriangle, CheckCircle2,
  Shield, Activity, RefreshCcw, TrendingUp, Stethoscope
} from "lucide-react";
      ...
      <div className="space-y-6">
        ...
        <div className="grid sm:grid-cols-3 gap-4">
          <div className="bg-card border border-border rounded-xl p-4 text-center">
            <p className="text-sm text-muted-foreground mb-1">Umumiy holat</p>
          ...
          <div className="bg-card border border-border rounded-xl p-4 text-center">
            <p className="text-sm text-muted-foreground mb-1">Turmush tarzi</p>
            <p className="text-xl font-bold text-foreground">{result.lifestyleScore}/100</p>
            <Progress value={result.lifestyleScore} className="mt-2 h-2" />
          </div>
        </div>

        {/* Risks */}
        <div className="space-y-3">
          <h3 className="font-semibold text-foreground">Kasallik xavflari</h3>
        ...
        <div className="bg-card border border-border rounded-xl p-4">
          <h3 className="font-semibold text-foreground mb-2">💡 Umumiy tavsiyalar</h3>
        ...
        {result.suggestedCheckups.length > 0 && (
          <div className="bg-primary/5 border border-primary/20 rounded-xl p-4">
            <h3 className="font-semibold text-foreground mb-2">🏥 Tavsiya etilgan tekshiruvlar</h3>
    ...
    <div className="space-y-6">
      ...
      <div className="grid sm:grid-cols-2 gap-4">
        ...
        <div>
          ...
          <select value={gender} onChange={(e) => setGender(e.target.value)} className="w-full h-10 rounded-md border border-input bg-background px-3 text-sm">
        ...
        <div>
          ...
          <select value={smoking} onChange={(e) => setSmoking(e.target.value)} className="w-full h-10 rounded-md border border-input bg-background px-3 text-sm">
            ...
            <option value="former">Avval chekganman</option>
        ...
        <div>
          ...
          <select value={alcohol} onChange={(e) => setAlcohol(e.target.value)} className="w-full h-10 rounded-md border border-input bg-background px-3 text-sm">
            <option value="">Tanlang</option>
            <option value="never">Ichmayman</option>
            <option value="rarely">Kam</option>
            <option value="moderate">O'rtacha</option>
            <option value="heavy">Ko'p</option>
          </select>
        </div>
        <div>
          <label className="text-sm font-medium text-foreground mb-1 block">Jismoniy faollik</label>
      ...
      <Button onClick={handleAnalyze} disabled={isLoading || !age || !gender} className="w-full" size="lg">
        {isLoading ? <><Loader2 className="w-5 h-5 mr-2 animate-spin" />Tahlil qilinmoqda...</> : <><HeartPulse className="w-5 h-5 mr-2" />Xavfni baholash</>}
      </Button>
    </div>
  );
};
