        ...
        <div className="inline-flex items-center gap-2 bg-muted rounded-full p-1">
          <button
            ...
            className={cn("px-4 py-2 rounded-full text-sm font-medium transition-all",
              billing === "monthly" ? "bg-primary text-primary-foreground shadow-md" : "text-muted-foreground"
            )}
          >Oylik</button>
          <button
            onClick={() => setBilling("yearly")}
            className={cn("px-4 py-2 rounded-full text-sm font-medium transition-all",
              billing === "yearly" ? "bg-primary text-primary-foreground shadow-md" : "text-muted-foreground"
            )}
          >Yillik <span className="text-xs opacity-80">(-17%)</span></button>
        </div>
      </div>
            ...
            >
              {plan.is_popular && (
                <div className="absolute -top-3 left-1/2 -translate-x-1/2">
                  <span className="inline-flex items-center gap-1 bg-hero-gradient text-primary-foreground text-xs font-bold px-3 py-1 rounded-full shadow-lg">
              ...
              <div className="mb-4">
                {price > 0 ? (
                  <div>
                    <span className="text-3xl font-extrabold text-foreground">{price.toLocaleString()}</span>
                    <span className="text-sm text-muted-foreground"> so'm/{billing === "monthly" ? "oy" : "yil"}</span>
                  </div>
                ) : (
              ...
              >
                {price === 0 ? "Joriy tarif" : "Obuna bo'lish"} <ArrowRight className="w-4 h-4 ml-1" />
              </Button>
            </div>
          );
        })}
