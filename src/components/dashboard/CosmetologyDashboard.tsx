...
interface CosmetologyCenter {
  ...
  website: string; telegram: string;
}
...
const CosmetologyDashboard = () => {
  ...
  const loadData = async () => {
    ...
    if (centers?.length) {
      ...
      const [svcRes, aptRes, photoRes] = await Promise.all([
        ...
        supabase.from("cosmetology_photos" as any).select("*").eq("center_id", c.id).order("sort_order") as any,
      ]);
  ...
  const saveProfile = async () => {
    if (!center) return;
    setSaving(true);
    const { error } = await supabase.from("registered_cosmetology" as any).update({
  ...
  const uploadPhoto = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    const path = `cosmetology-photos/${center.id}/${Date.now()}.${ext}`;
    ...
    await supabase.from("cosmetology_photos" as any).insert({ center_id: center.id, url: data.publicUrl, sort_order: photos.length } as any);
    toast({ title: "✅ Rasm yuklandi" }); loadData();
  };

  const deletePhoto = async (id: string) => {
    await supabase.from("cosmetology_photos" as any).delete().eq("id", id);
        ...
        <TabsContent value="profile" className="space-y-4">
          ...
          <Card><CardContent className="p-5 space-y-4">
            {profileEdit ? (
              <>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div><Label>Markaz nomi</Label><Input value={profileForm.name} onChange={e => setProfileForm(p => ({ ...p, name: e.target.value }))} className="mt-1" /></div>
