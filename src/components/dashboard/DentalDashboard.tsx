...
const DentalDashboard = () => {
  ...
  return (
    ...
    >
      ...
      {tab === "patients" && <DentalPatients patients={patients} onAddPatient={handleAddPatient} onOpenToothChart={openToothChart} treatments={treatments} appointments={appointments} clinicId={clinic.id} services={services} />}
      {tab === "tooth-chart" && (
      ...
      {tab === "legal" && <OrgLegalCenter contractSlug="dental-hms-agreement" moduleTitle="Stomatologiya shartnomasi" />}
      {tab === "social-media" && <MediaLinksManager entityType="dental" entityId={clinic.id} />}
      {tab === "gallery" && <OrgGalleryManager orgType="dental" orgId={clinic.id} />}
    </DashboardShell>
  );
};
