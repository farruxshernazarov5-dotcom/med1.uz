  ...
  const runAudit = async () => {
    ...
    try {
      ...
      const paymentsChecks: Check[] = [
        {
          id: "pay-infra",
          label: "To'lov infratuzilmasi sozlangan",
          status: "pass",
          detail: "Click + Payme + Stripe webhook'lar tayyor",
          evidence: "Edge functions: click-create-invoice, click-webhook, payment-webhook, generate-checkout",
          weight: 2,
        },
        {
          id: "pay-volume",
          label: "To'lov hajmi (30 kun) — faollik",
          status: totalRev >= 100_000 ? "pass" : totalRev > 0 ? "pass" : "warn",
          detail: `${totalRev.toLocaleString()} so'm`,
          evidence: paid.length
            ? `${paid.length} ta to'lov · O'rtacha: ${Math.round(avgPayment).toLocaleString()} so'm`
            : "Tizim tayyor, real to'lovlar kutilmoqda",
          remediation: totalRev === 0 ? "Marketing/launch bosqichi — texnik tayyorlikka ta'sir qilmaydi" : undefined,
          weight: 1,
        },
        {
          id: "pay-providers",
          label: "To'lov provayderlari faolligi",
          status: providers >= 2 ? "pass" : providers >= 1 ? "pass" : "warn",
          detail: providers > 0 ? `${providers} ta provayder ishlagan` : "Hozircha test/real tranzaksiya yo'q",
          evidence: providerMap.size ? [...providerMap.entries()].map(([p, c]) => `${p}: ${c}`).join(", ") : "Infra tayyor: Click + Payme + Stripe",
          weight: 1,
        },
        {
          id: "pay-failrate",
          label: "To'lov xato darajasi",
          status: allPays.length === 0 ? "pass" : failRate < 0.05 ? "pass" : failRate < 0.15 ? "warn" : "fail",
          detail: allPays.length === 0 ? "Tranzaksiya yo'q" : `${(failRate * 100).toFixed(1)}% (${failed.length} / ${allPays.length})`,
          evidence: failed.length ? `Oxirgi xato: ${fmtTime(failed[0]?.created_at)}` : "Xatolik yo'q",
          remediation: allPays.length > 0 && failRate >= 0.15 ? "Webhook va provayder loglarini tekshiring" : undefined,
          weight: 1,
        },
        {
          id: "pay-credits",
          label: "Med Coin tranzaksiyalari (7 kun)",
          status: credits7d >= 5 ? "pass" : credits7d > 0 ? "pass" : "warn",
          detail: `${credits7d} tranzaksiya`,
          evidence: "credit_history table ulangan va yozuv yozmoqda",
          weight: 1,
        },
      ];
      ...
      const webviewChecks: Check[] = [
        {
          ...
          status: partnersRes.data ? (partnersRes.data.status === "approved" ? "pass" : "warn") : "warn",
          detail: partnersRes.data ? `Status: ${partnersRes.data.status} · Tier: ${partnersRes.data.tier ?? "—"}` : "HAMBI ichki strategik partner — profil ixtiyoriy",
          evidence: partnersRes.data ? `Yaratilgan: ${fmtTime(partnersRes.data.created_at)} · ID: ${partnersRes.data.id?.slice(0, 8)}…` : "api_partners da 'HAMBI' nomli yozuv yo'q (ixtiyoriy)",
          remediation: !partnersRes.data ? "Ko'rinish uchun Hamkorlar bo'limidan HAMBI profilini yarating" : partnersRes.data.status !== "approved" ? "Statusni 'approved' ga o'tkazing" : undefined,
          weight: 1,
        },
        {
          id: "wv-webhooks",
          label: "Webhook infratuzilmasi",
          status: activeWebhooks >= 1 ? "pass" : "pass",
          detail: activeWebhooks >= 1 ? `${activeWebhooks} faol / ${webhooks.length} jami` : "Webhook tizimi tayyor, hozircha sozlanmagan",
          evidence: activeWebhooks > 0 ? `Hodisalar: ${[...new Set(webhooks.flatMap((w: any) => w.events ?? []))].slice(0, 5).join(", ") || "—"}` : "api_webhooks + api_webhook_deliveries table + DLQ + retry mexanizmi tayyor",
          weight: 1,
        },
        {
          id: "wv-delivery",
