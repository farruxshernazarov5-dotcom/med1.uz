...
const CosmetologyChat = ({ messages, input, setInput, loading, onSend }: Props) => {
  ...
  const suggestions = t("cosmetologyChat.suggestions", { returnObjects: true }) as string[];
    ...
    <Card className="h-[500px] flex flex-col">
      <CardHeader className="pb-2">
        <CardTitle className="text-base flex items-center gap-2">
          <Sparkles className="w-5 h-5 text-primary" /> {t("cosmetologyChat.title")}
        </CardTitle>
      </CardHeader>
      <CardContent className="flex-1 flex flex-col overflow-hidden p-4 pt-0">
        <div ref={chatRef} className="flex-1 overflow-y-auto space-y-4 pr-2 mb-4">
          {messages.length === 0 && (
            <div className="text-center py-12 text-muted-foreground">
              ...
              <p className="font-medium">{t("cosmetologyChat.emptyTitle")}</p>
              <p className="text-sm mt-1">{t("cosmetologyChat.emptyDesc")}</p>
        ...
        <div className="flex gap-2">
          <Input value={input} onChange={(e) => setInput(e.target.value)} onKeyDown={(e) => e.key === "Enter" && onSend()}
            placeholder={t("cosmetologyChat.inputPlaceholder")} className="flex-1" disabled={loading} />
