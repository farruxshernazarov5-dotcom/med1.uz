...
import { Plus, X, Phone, Clock, Users, ArrowRight } from "lucide-react";
...
const HMSQueue = ({ clinicId }: Props) => {
  ...
  const fetchData = async () => {
    const [qRes, patRes, docRes, deptRes] = await Promise.all([
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-4">
        <h2 className="font-heading text-xl font-bold text-foreground">Navbat ({queue.length})</h2>
      ...
      {called.length > 0 && (
        <div className="mb-6">
          <h3 className="font-heading font-bold text-foreground mb-3 flex items-center gap-2"><Phone className="w-4 h-4 text-primary" /> Chaqirilganlar</h3>
            ...
            {called.map(q => (
              <div key={q.id} className="bg-primary/5 border border-primary/20 rounded-xl p-4 flex flex-col sm:flex-row sm:items-center gap-3">
                ...
                <div className="flex gap-2">
                  <Button size="sm" onClick={() => complete(q.id)}>Tugallash</Button>
                  <Button size="sm" variant="outline" onClick={() => skip(q.id)}>O'tkazish</Button>
                </div>
              </div>
            ))}
          </div>
      ...
      <div className="space-y-2">
        {waiting.map(q => (
          <div key={q.id} className="bg-card rounded-xl border border-border p-4 flex flex-col sm:flex-row sm:items-center gap-3">
            <div className="flex items-center gap-3 flex-1">
              <div className="w-10 h-10 rounded-xl bg-muted flex items-center justify-center font-bold text-foreground">#{q.queue_number}</div>
              <div>
                <p className="font-semibold text-foreground text-sm">{q.patient_name}</p>
            ...
            <div className="flex items-center gap-2">
              <Badge className={cn("text-[10px]", priorityColors[q.priority])}>{q.priority}</Badge>
              ...
              <Button size="sm" variant="ghost" className="text-xs h-7" onClick={() => skip(q.id)}>O'tkazish</Button>
            </div>
          </div>
        ))}
      </div>
