...
import { Plus, FileText, Heart, Search, X, Trash2, Upload, Download, Edit2 } from "lucide-react";
...
const HMSFilesAndDonors = ({ clinicId }: Props) => {
  ...
  const categoryLabels: Record<string, string> = { general: "Umumiy", report: "Hisobot", contract: "Shartnoma", license: "Litsenziya", medical: "Tibbiy", financial: "Moliyaviy" };

  return (
    <div>
      <Tabs defaultValue="files" className="w-full">
        <TabsList className="grid grid-cols-2 w-full max-w-sm mb-6">
          <TabsTrigger value="files"><FileText className="w-4 h-4 mr-1" /> Fayllar</TabsTrigger>
          <TabsTrigger value="donors"><Heart className="w-4 h-4 mr-1" /> Donorlar</TabsTrigger>
        </TabsList>
        ...
        <TabsContent value="files">
          <div className="flex flex-col sm:flex-row gap-3 mb-4">
            ...
            <Button size="sm" onClick={() => setShowFileForm(true)}><Plus className="w-4 h-4 mr-1" /> Yangi fayl</Button>
          </div>
            ...
            {filteredFiles.map(f => (
              <div key={f.id} className="bg-card rounded-xl border border-border p-4 flex flex-col sm:flex-row sm:items-center gap-3">
                ...
                <div className="flex-1">
                  <p className="font-semibold text-foreground text-sm">{f.file_name}</p>
                  <div className="flex gap-2 text-xs text-muted-foreground">
                    <Badge variant="outline" className="text-[10px]">{categoryLabels[f.category] || f.category}</Badge>
                ...
                <div className="flex gap-1">
                  ...
                  <Button variant="ghost" size="icon" onClick={() => handleDeleteFile(f.id)}><Trash2 className="w-4 h-4 text-destructive" /></Button>
                </div>
              </div>
            ))}
        ...
        <TabsContent value="donors">
          ...
          <div className="space-y-2">
            {filteredDonors.map(d => (
              <div key={d.id} className="bg-card rounded-xl border border-border p-4 flex flex-col sm:flex-row sm:items-center gap-3">
                <Heart className="w-5 h-5 text-red-500 shrink-0" />
                <div className="flex-1">
                  <p className="font-semibold text-foreground text-sm">{d.full_name}</p>
                  <div className="flex flex-wrap gap-2 text-xs text-muted-foreground">
                    <span>{d.phone}</span>
                    <Badge variant="outline" className="text-[10px] font-bold">{d.blood_group}{d.rh_factor}</Badge>
                    <span>{d.donation_count || 0} marta topshirgan</span>
                    {d.last_donation_date && <span>Oxirgi: {d.last_donation_date}</span>}
                  </div>
                </div>
                <div className="flex gap-1">
                  <Button size="sm" variant="outline" className="text-xs" onClick={() => handleRecordDonation(d.id)}>
                    <Heart className="w-3 h-3 mr-1" /> Donatsiya
                  </Button>
                  <Button variant="ghost" size="icon" onClick={() => { setEditingDonor(d); setDonorForm({ full_name: d.full_name, phone: d.phone, blood_group: d.blood_group, rh_factor: d.rh_factor, gender: d.gender, date_of_birth: d.date_of_birth || "", notes: d.notes || "" }); setShowDonorForm(true); }}>
                    <Edit2 className="w-4 h-4" />
                  </Button>
                  <Button variant="ghost" size="icon" onClick={() => handleDeleteDonor(d.id)}>
