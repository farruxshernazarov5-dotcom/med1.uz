...
const BIExecutive = ({ m, onDrill }: { m: BIMetrics; onDrill?: (k: string) => void }) => {
  const kpis = useMemo(() => {
    ...
    return [
      ...
      { icon: TrendingUp, label: "Xarajatlar", value: fmtMoney(m.expenses), suffix: "so'm", delta: null, health: "good", key: "finance" },
      { icon: AlertTriangle, label: "Qarzdorlik", value: fmtMoney(m.outstanding), suffix: "so'm", delta: null, health: (m.outstanding > m.grossRevenue * 0.2 ? "bad" : "good"), key: "finance" },
      { icon: Calendar, label: "Qabullar", value: m.appts.length, delta: apptDelta, health: healthFromDelta(apptDelta), key: "appointments" },
      { icon: CheckCircle2, label: "Yakunlangan", value: m.completed, delta: null, health: "good", key: "appointments" },
      { icon: XCircle, label: "Bekor qilingan", value: m.cancelled, delta: null, health: (m.cancelled > m.appts.length * 0.15 ? "warn" : "good"), key: "appointments" },
      { icon: UserX, label: "No-show", value: `${m.noShow}`, suffix: `(${m.noShowRate}%)`, delta: null, health: (m.noShowRate > 10 ? "bad" : m.noShowRate > 5 ? "warn" : "good"), key: "appointments" },
      { icon: Users, label: "Jami bemorlar", value: m.uniquePatients, delta: null, health: "good", key: "patients" },
      { icon: UserPlus, label: "Yangi bemorlar", value: m.newPatients, delta: null, health: "good", key: "patients" },
      { icon: Repeat, label: "Qayta kelgan", value: m.returningPatients, delta: null, health: "good", key: "patients" },
      { icon: Clock, label: "O'rtacha kutish", value: m.avgWaitMinutes, suffix: "daq", delta: null, health: (m.avgWaitMinutes > 30 ? "bad" : m.avgWaitMinutes > 15 ? "warn" : "good"), key: "operations" },
      { icon: Timer, label: "Qabul davomiyligi", value: m.avgDurationMinutes, suffix: "daq", delta: null, health: "good", key: "operations" },
      { icon: Stethoscope, label: "Shifokor bandligi", value: m.doctorUtilization, suffix: "%", delta: null, health: (m.doctorUtilization > 85 ? "warn" : "good"), key: "doctors" },
      { icon: BedDouble, label: "Palata bandligi", value: `${m.bedsOccupied}/${m.bedsTotal}`, suffix: `${m.bedOccupancy}%`, delta: null, health: "good", key: "operations" },
      { icon: FlaskConical, label: "Laboratoriya", value: m.labCount, suffix: "buyurtma", delta: null, health: (m.labLate > 0 ? "warn" : "good"), key: "operations" },
      { icon: Pill, label: "Dorixona savdosi", value: fmtMoney(m.pharmacySales), suffix: "so'm", delta: null, health: "good", key: "operations" },
      { icon: TrendingUp, label: "O'rtacha chek", value: fmtMoney(m.completed ? m.grossRevenue / m.completed : 0), suffix: "so'm", delta: null, health: "good", key: "finance" },
    ];
  }, [m]);
