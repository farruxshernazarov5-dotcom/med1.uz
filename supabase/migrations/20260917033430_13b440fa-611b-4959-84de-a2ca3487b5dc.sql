...
BEGIN
  ...
  -- 1) Paketni aniqlash: package_id -> purpose ichidagi kod -> summa bo'yicha
  ...
  IF pkg.id IS NOT NULL AND pkg.kind = 'med_coin' THEN
    ...
    VALUES (p.user_id, total_coins, 'purchase', pkg.name_uz || ' (' || p.provider || ')', bal_after);
  ...
  ELSIF pkg.id IS NOT NULL AND pkg.kind = 'subscription' THEN
    ...
    IF total_coins > 0 THEN
      ...
      bal_after := bal_before + total_coins;
      INSERT INTO public.credit_history (user_id, amount, type, description, balance_after)
      VALUES (p.user_id, total_coins, 'purchase', pkg.name_uz || ' (obuna)', bal_after);
    END IF;
  ELSE
...
-- Foydalanuvchi o'z to'lovi uchun fulfillment'ni qayta tekshirishi mumkin (faqat o'zinikini)
CREATE OR REPLACE FUNCTION public.claim_my_payment(_payment_id uuid)
RETURNS jsonb
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = public
...
DECLARE
  owner uuid;
BEGIN
  SELECT user_id INTO owner FROM public.platform_payments WHERE id = _payment_id;
  IF owner IS NULL OR owner <> auth.uid() THEN
    RETURN jsonb_build_object('ok', false, 'error', 'forbidden');
  END IF;
  RETURN public.fulfill_platform_payment(_payment_id);
END;
$$;

GRANT EXECUTE ON FUNCTION public.claim_my_payment(uuid) TO authenticated;

-- Avval to'langan, lekin xizmat berilmagan to'lovlarni qayta ishlash
DO $$
DECLARE r RECORD;
BEGIN
  FOR r IN SELECT id FROM public.platform_payments
            WHERE status IN ('paid','completed') AND fulfilled_at IS NULL
  LOOP
    PERFORM public.fulfill_platform_payment(r.id);
  END LOOP;
END $$;
