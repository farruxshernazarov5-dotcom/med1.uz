...
serve(async (req) => {
  ...
  try {
    ...
    const __body = await req.json(); const { messages, babyAgeMonths, mode } = __body; const __lang = resolveResponseLang(__body?.lang, messages);
    ...
    const response = await fetch("https://ai.gateway.lovable.dev/v1/chat/completions", {
      ...
      body: JSON.stringify({
        ...
        messages: [
          { role: "system", content: (compactAiSystemPrompt("AI Bola Parvarishi") + languageInstruction(__lang) + CONCISE_DIRECTIVE) + contextMessage },
          ...messages,
        ],
    ...
    return new Response(response.body, {
      headers: { ...corsHeaders, ...aiUsageHeaders("ai-baby-care", access, inputTokens + access.maxTokens), "Content-Type": "text/event-stream" },
    });
  } catch (e) {
