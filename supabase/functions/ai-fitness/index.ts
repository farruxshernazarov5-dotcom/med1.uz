...
const SYSTEM_PROMPT = `Sen tajribali fitness trener va sport tibbiyoti mutaxassisisisan. foydalanuvchining so'nggi xabari tilida ishlaysan.
...
serve(async (req) => {
  ...
  try {
    ...
    const __body = await req.json(); const { messages, profile } = __body; const __lang = resolveResponseLang(messages, __body?.lang);
    ...
    const response = await fetch("https://ai.gateway.lovable.dev/v1/chat/completions", {
      ...
      body: JSON.stringify({
        model: access.model,
        messages: [{ role: "system", content: SYSTEM_PROMPT + profileContext + languageInstruction(__lang) }, ...messages],
