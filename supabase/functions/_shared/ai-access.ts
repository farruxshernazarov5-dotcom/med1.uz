...
export async function enforceAiAccess(req: Request, serviceId: string): Promise<AiAccessResult> {
  try {
    ...
    /* ─── MONTHLY FREE GRANT (2 coins/month) is now credited to real balance
     * via `grant_monthly_free_coins` RPC on client login. Deduction is STRICT:
     * no bypass — if balance is 0 the request is rejected below.
     * Best-effort top-up here for server-only callers (e.g. mobile channels)
     * so a first-time user still receives the grant even if the client
     * hasn't called the RPC yet.
     */
    try {
    ...
    if (!row?.success) {
      return { allowed: false, status: 402, error: row?.error || "Med Coin yetarli emas. Oylik 2 ta bepul so'rov tugagan bo'lsa, Med Coin sotib oling yoki tarifni yangilang." };
    }
